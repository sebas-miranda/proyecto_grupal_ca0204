...
```{r}
resumen_subdelito <-  read_csv(here("data/processed/resumen_subdelito.csv"), show_col_types = FALSE)
head(resumen_subdelito, 10)
```

Análogamente, estos son los diez subdelitos más frecuentes que se reportan en Costa Rica desde 2019 hasta 2025. Liderando los descuidos, el arma de fuego, la forzadura, la tacha de vehículo y el hurto. Podemos observar que la tendencia tiene una intrínsica relación con los delitos reportados (Pues son sucesos secundarios del delito principal).
...
```{r}
resumen_subvictima <-  read_csv(here("data/processed/resumen_subvictima.csv"), show_col_types = FALSE)
head(resumen_subvictima, 10)
```

Estos son las diez subvíctimas implicadas en los delitos reportados. Como puede observar, más de la mitad de víctimas implicadas en los delitos reportados no son como tal personas. Por ende, se comprende que los daños colaterales de los eventos son los bienes de las víctimas. Cabe recalcar que más de 50 000 peatones son víctimas frecuentes de los delitos. Esto tiene relación con los hurtos, robos y asaltos, que son la mayoría de eventos registrados. Por lo tanto, se entiende que gran parte de los delitos cometidos ocurren en sitios públicos.
