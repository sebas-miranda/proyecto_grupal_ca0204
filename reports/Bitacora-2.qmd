...
grafico_fecha_frecuencia <- datos %>% 
  ...
  mutate(frecuencia_miles = frecuencia / 1000) %>% #Antes el grafico se veia incosistente con los numeros de casos correspondiente por ende decidi trabajar las frecuencias en miles
...
Tome en cuenta que el proyecto se compone de variables categoricas, que en su mayoria fueron consideradas en los puntos 4 y 5. Por lo tanto para este insiso unicamente se tomo en cuenta las fechas.
...
Ahora, para identificar outliers se va a utilizar el método IQR (Interquartile Range o Rango Intercuartílico) la cual es una técnica estadística utilizada para identificar valores atípicos en un conjunto de datos. Un valor se considera outlier si se encuentra fuera de los límites:𝑥\< Q1 − 1.5 × IQR o𝑥\> Q3 + 1.5 × IQR. Donde IQR=Q3-Q1. Este método es robusto porque no se ve afectado por la asimetría o valores extremos aislados. En este proyecto se utilizará el criterio del 1.5 × IQR, que es el mismo que emplean los gráficos de caja (boxplots) para detectar valores inusualmente altos o bajos en las frecuencias de delitos.
...
En el punto anterior se observaron muchos outliers, los cual se debe corregir. La winsorización es una técnica estadística utilizada para reducir el efecto de valores extremos sin eliminarlos del conjunto de datos. Consiste en reemplazar los valores que superan los límites definidos (ya sea por percentiles o por el rango intercuartílico) por el valor más cercano dentro de esos límites. En este proyecto se aplicará una winsorización basada en el método IQR (1.5 × IQR), con el fin de suavizar la influencia de los cantones o delitos con frecuencias extremadamente altas, como San José o Hurto, los cuales, aunque representan fenómenos reales, podrían distorsionar la visualización y el análisis estadístico. De esta forma se conserva la información esencial sin perder la coherencia general de la distribución.
...
Organismo de Investigación Judicial (OIJ). (2025). *Datos Abiertos*. Organismo de Investigación Judicial. https://sitiooij.poder-judicial.go.cr/index.php/ayuda/servicios-policiales/servicios-a-organizaciones/indice-de-transparencia-del-sector-publico-costarricense/datos-abiertos

ggplot. (2020). *Cartesian coordinates with x and y flipped*. Gggplot2. https://ggplot2.tidyverse.org/reference/coord_flip.html\
\
