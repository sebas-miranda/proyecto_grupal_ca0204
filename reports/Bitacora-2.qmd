...
Ahora, para identificar outliers se va a utilizar el método IQR (Interquartile Range o Rango Intercuartílico) la cual es una técnica estadística utilizada para identificar valores atípicos en un conjunto de datos. Un valor se considera outlier si se encuentra fuera de los límites:𝑥< Q1 − 1.5 × IQR o𝑥> Q3 + 1.5 × IQR. Donde IQR=Q3-Q1.
Este método es robusto porque no se ve afectado por la asimetría o valores extremos aislados. En este proyecto se utilizará el criterio del 1.5 × IQR, que es el mismo que emplean los gráficos de caja (boxplots) para detectar valores inusualmente altos o bajos en las frecuencias de delitos.
...
o_canton    <- outliers_q1q3_std(resumen_canton,       "canton",       "frecuencia_de_cantones",       "resumen_canton")
o_delito    <- outliers_q1q3_std(resumen_delito,       "delito",       "frecuencia_de_delitos",        "resumen_delito")
o_edad      <- outliers_q1q3_std(resumen_edad,         "edad",         "frecuencia_rango_de_edades",   "resumen_edad")
o_nacional  <- outliers_q1q3_std(resumen_nacionalidad, "nacionalidad", "frecuencia_de_nacionalidades", "resumen_nacionalidad")
o_provincia <- outliers_q1q3_std(resumen_provincia,    "provincia",    "frecuencia_de_provincias",     "resumen_provincia")
o_sexo      <- outliers_q1q3_std(resumen_sexo,         "sexo",         "frecuencia_de_sexo",           "resumen_sexo")
o_victima   <- outliers_q1q3_std(resumen_victima,      "victima",      "frecuencia_de_victimas",       "resumen_victima")
